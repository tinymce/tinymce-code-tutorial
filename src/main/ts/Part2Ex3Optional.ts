...
export const nonEmptyString = (s: string): Optional<string> => {
  return s.length > 0 ? Optional.some(s) : Optional.none();
};
...
export const getProtocol = (url: string): Optional<string> => {
  ...
  return protocol && isAlpha(protocol[0])
    ? Optional.some(protocol[0])
    : Optional.none();
};

const isAlpha = (s: string): boolean => /^[a-zA-Z]+$/i.test(s);
...
export const getAttribute = (e: Element, attrName: string): Optional<Attr> =>
  Optional.from(e.getAttributeNode(attrName));
...
export const optionalDouble = (n: Optional<number>): number =>
  n.fold(
    () => 0,
    (num) => num * 2
  );
...
export const optionalToArray = <A>(opt: Optional<A>): A[] =>
  opt.fold(
    () => [],
    (s) => [s]
  );
...
export const addThreeToOptional = (num: Optional<number>): Optional<number> =>
  num.map((x) => x + 3);

// DONE: Write a function that takes an Optional<string> and prefixes the string with "hello"
export const prefixStringOptional = (str: Optional<string>): Optional<string> =>
  str.map((x) => "hello" + x);
...
Answer: Returns Optional.none<string>(). Based on definitions, map called on an Optional.none() will return Optional.none() immediately.
...
+
+ Our "fold" function destructures the Optional type. In category theory, "fold" is known as a "catamorphism".
+
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
